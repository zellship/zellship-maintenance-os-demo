...
export function Resources() {
  ...
  const activeReservations = reservations.filter(
    (r) => r.status === "Reserved" || r.status === "InUse",
  );
  const stockAlerts = inventory.filter(
    (i) => i.onHand - i.reserved - i.quarantine <= i.reorderPoint,
  );
    ...
    <div className="industrial-page">
      <div style={{ marginBottom: 16 }}>
        ...
        <Typography.Text type="secondary">
          Elegibilidad, disponibilidad y reservas conectadas a cada orden de mantenimiento.
        </Typography.Text>
      </div>
        ...
        <Col xs={12} lg={6}>
          <Card>
            <Statistic
              ...
              value={tools.filter((t) => t.status === "Available").length}
              suffix={`/ ${tools.length}`}
              prefix={<ToolOutlined />}
            />
          </Card>
                  ...
                  columns={[
                      ...
                      render: (name, person) => (
                        <Space direction="vertical" size={0}>
                          ...
                          <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                            {person.plant} · Turno{" "}
                            {person.shift === "Morning" ? "mañana" : person.shift}
                          </Typography.Text>
                        </Space>
                      ),
                    },
                    {
                      ...
                      render: (ids) => (
                        <Space wrap>
                          {ids.map((id: string) => (
                            <Tag key={id}>{seedSkills.find((s) => s.id === id)?.name || id}</Tag>
                          ))}
                        </Space>
                      ),
                    },

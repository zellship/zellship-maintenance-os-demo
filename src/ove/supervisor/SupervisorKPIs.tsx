...
export function SupervisorKPIs() {
  ...
  const filteredExecutions = executions.filter((e) => {
    ...
    return (
      (!plant || schedule?.plant === plant) &&
      (!shift || shiftValue === shift) &&
      (!operator || e.operator === operator) &&
      periodOk
    );
  });
  ...
  const byOperator = operators.map((op) => {
    ...
    return {
      ...
      total: ex.length,
      validated: ok,
      compliance: ex.length ? Math.round((ok / ex.length) * 100) : 0,
    };
  });

  const topIncidents = protocols
    .map((p) => ({
      key: p.id,
      name: p.name,
      count: incidents.filter((i) => i.protocolId === p.id).length,
    }))
  ...
  const avgDuration = (() => {
    ...
    const mins = done.reduce(
      (acc, e) => acc + (new Date(e.endAt!).getTime() - new Date(e.startAt).getTime()) / 60000,
      0,
    );
    ...
    <div>
      ...
      <Space wrap style={{ marginBottom: 16 }}>
        ...
        <Select
          ...
          value={shift}
          onChange={setShift}
          options={["Mañana", "Tarde", "Noche"].map((s) => ({ label: s, value: s }))}
          style={{ width: 140 }}
          allowClear
        />
        <Select
          ...
          value={period}
          onChange={setPeriod}
          options={["Hoy", "Semana", "Mes"].map((s) => ({ label: s, value: s }))}
          style={{ width: 140 }}
          allowClear
        />
        <Select
      ...
      <Row gutter={[16, 16]}>
        <Col xs={24} md={6}>
          <Card>
            <Statistic
              title="Tiempo promedio de intervención"
              value={avgDuration}
              suffix="min"
        ...
        <Col xs={24} md={6}>
          <Card>
            <Statistic
              ...
              value={incidents.filter((i) => i.status === "Open" || i.status === "Review").length}
          ...
          <Card title="Protocolos con desviaciones">
            <Table
              ...
              columns={[
                ...
                { title: "Incidencias", dataIndex: "count", width: 110 },
              ]}
            />
          </Card>
        </Col>
